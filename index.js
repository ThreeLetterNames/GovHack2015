/*property
    ...
    send, show, sin, sort, split, sqrt, src, substr, text, title, toLowerCase
*/
/*global console */
"use strict";
...
//        console.log("heritage: ..."+data.substr(10,50)+"... ("+lines.length+"lines)");
        for(line = 0; line < lines.length; line += 1) {
            ...
            if (line !== 0 && fields.length > 5) {
                t = fields[1];
                d = fields[2]+" ("+fields[5]+", "+fields[4]+": "+fields[6]+" - "+fields[7]+" - "+fields[17]+" - "+fields[18]+" - "+fields[19]+" - "+fields[26]+" - "+fields[27]+")" ;
...
//                console.log("undef:"+allData.length); //not ready untill user interacts
    ...
    // search data an sort by distance (with areas priooritized first) /////
            ...
            var working_div, i, link_text, link_count;
